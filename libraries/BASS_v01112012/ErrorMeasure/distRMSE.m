function [DRMSE,DRMSE_active, Ndif] = distRMSE(Q,STrue,S,vi,opts)
%==========================================================================
...
Sw = repmat(sqrt(min(EdistTable,[],2)),[1 Nt]).*S;
Ndif = length(find(sum(S.^2,2)));

DRMSE = sqrt( Sw(:)'*Sw(:) );
DRMSE_active = DRMSE/(Ndif*(S(:)'*S(:)));