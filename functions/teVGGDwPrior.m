function [V,m,X,FreeEnergy,f,M,beta,fval] = teVGGDwPrior(A,Y,mold,gamma,opts)
%
...
% Settings:
try max_iter = opts.max_iter; catch; max_iter = 10000; end; % Maximum number of iterations
...
try pn = opts.pnorm; catch; pn=0.8; end; 
...
while k <max_iter && term==0
    ...
    h=(fact*2*mold)*gamma;
    % keyboard
    dFdm = h-gamma+beta*K/2*sum(repmat((1-2*m).*chi_nn,1,T).*X2,2)+log(m./(1-m))-sum(X.*(A'*lambda),2);
...
FreeEnergy = f(k);
V = X.*repmat(m,1,T);
%% Calculate Free energy on validation
...
if nargout>7
    ...
    C = eye(Kv)+1/Kv*opts.A_val*spdiags(m./(1-m)./chi_nn,0,N,N)*opts.A_val';
    yhat = C\opts.Y_val;
    yhaty = yhat.*opts.Y_val;
    beta = T*Kv/sum(yhaty(:));
    lambda = beta*yhat;
    z = opts.Y_val-1/beta*lambda;
    
    fval = -T*Kv/2*log(beta/(2*pi))+beta/2*sum(sum((z-opts.Y_val).^2))...
        +Kv*beta/2*sum(sum(X2.*repmat(chi_nn.*m.*(1-m),1,T)))...
        -gamma*sum(m)+N*log(1+exp(gamma))...
        +sum(m.*log(m)+(1-m).*log(1-m))...
        +sum(sum(lambda.*(z-opts.A_val*(repmat(m,1,T).*X)))); % Free energy
end
